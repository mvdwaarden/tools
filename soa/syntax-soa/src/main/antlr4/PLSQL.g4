...
datatype
    : ( REF )? ID ( DOT ID )? ( LPAREN (numeric_literal ( COMMA numeric_literal )*)(ID)? RPAREN | PERCENT ( kTYPE | ROWTYPE ) )?
    ;
...
pipe_row_statement : 
	    PIPE ROW LPAREN parameter RPAREN 
;
...
interval_literal
    : INTERVAL QUOTED_STRING ID (LPAREN numeric_literal ( COMMA numeric_literal ) RPAREN)? (TO ID)?
    ;
